...
/**
 * Milestone 9: same demo flow as /demo/ksl-lstm, wired to
 * backend_keras3's GRU model instead. Every hook and component here
 * is reused unchanged -- keypoint extraction is imported from
 * lib/keras3KeypointsApi (shared, model-agnostic) and predictSequence
 * is lib/keras3PredictionApi's generalized function, bound to "gru"
 * via the small wrapper below rather than duplicating
 * usePredictionSubmission's call site logic.
 */
export default function KslGruDemo() {
      ...
      <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-8 px-6 py-10 sm:px-10 sm:py-14">
        <div className="flex flex-col gap-2">
          ...
          <h1 className="text-2xl font-semibold tracking-tight text-black sm:text-3xl dark:text-zinc-50">
            Khmer Sign Language Prediction Demo (GRU)
          </h1>
          <p className="max-w-xl text-sm leading-relaxed text-zinc-600 sm:text-base dark:text-zinc-400">
            This page uses backend_keras3&apos;s GRU model (pose + hands,
            20 classes).
          </p>
        </div>
