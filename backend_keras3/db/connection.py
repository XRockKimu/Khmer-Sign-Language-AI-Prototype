...
Milestone 12: identical in shape and behavior to backend/db/connection.py
-- this backend logs into the same PostgreSQL database (see
database/migrations/004_multi_model_unified_logging.sql, which added
backend/model_id columns precisely so both backends can share
prediction_events/model_versions). backend and backend_keras3 are
independent deployable services with separate virtualenvs and no shared
Python import path between them, so this is a small, deliberate copy
rather than a shared module -- matching how ai_inference/model_loader.py,
label_loader.py, and main.py are already separately maintained per
backend rather than imported across the two.
